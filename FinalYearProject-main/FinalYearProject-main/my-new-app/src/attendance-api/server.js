...
//       return res.status(404).json({ error: "No student connected to this account" });
//     }
...
// ---------------------------------------------------------
// ROUTE 2: THE CALLBACK (No Database Required)
// ---------------------------------------------------------
app.get('/auth/callback', async (req, res) => {
    ...
    try {
        // 1. Swap the code for the Microsoft Token
        const response = await pca.acquireTokenByCode(tokenRequest);
        
        // 2. Grab everything you need directly from Microsoft
        const microsoftAccount = response.account;
        
        // 3. PUFF! Save it straight to the session. No PostgreSQL needed.
        req.session.isAuthenticated = true;
        req.session.user = { 
            name: microsoftAccount.name, 
            email: microsoftAccount.username,
            id: microsoftAccount.homeAccountId // Microsoft's unique ID for them
        };

        // 4. Send them directly to the main page
